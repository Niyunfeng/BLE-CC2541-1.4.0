 ...
 * @fn      osal_pwrmgr_init
 *
 * @brief   Initialize the power management system. osal_init_system( )÷–µ˜”√
 *
 * @param   none.
 *
 * @return  none.
...
{
  pwrmgr_attribute.pwrmgr_device = PWRMGR_ALWAYS_ON; // Default to no power conservation.ƒ¨»œ√ª”–ÀØ√ﬂƒ£ Ω°£
  pwrmgr_attribute.pwrmgr_task_state = 0;            // Cleared.  All set to conserve «Â¡„
}
 ...
 *          not this task wants to conserve power.
     ’‚∏ˆ∫Ø ˝ø…“‘±ª√ø“ª∏ˆ»ŒŒÒµ˜”√£¨”√”⁄…Ë÷√’‚∏ˆ»ŒŒÒ «∑Ò÷ß≥÷µÕπ¶∫ƒ‘À––£¨»Á

 * π˚√ø“ª∏ˆ»ŒŒÒ≤ª÷ß≥÷µÕπ¶∫ƒΩ´Œﬁ∑®Ω¯»ÎµÕπ¶∫ƒƒ£ Ω‘À––°£
 *
...
uint8 osal_pwrmgr_task_state( uint8 task_id, uint8 state )
{
  ...
  { 
    // Clear the task state flag «Â¡„
    pwrmgr_attribute.pwrmgr_task_state &= ~(1 << task_id );
  }
  else
  {
    // Set the task state flag ÷√Œª
    pwrmgr_attribute.pwrmgr_task_state |= (1 << task_id);
  }
 ...
 *          no events scheduled and shouldn't be called from anywhere else.
 ’‚∏ˆ∫Ø ˝‘⁄OSAL—≠ª∑÷–»Áπ˚√ª”–»Œ∫Œ ¬º˛–Ë“™÷¥––µƒª∞Ω´±ªµ˜”√£¨Ω´…Ë±∏Ω¯»Î

* ÀØ√ﬂƒ£ Ω£¨≤ªø…“‘‘⁄∆‰À˚µÿ∑Ωµ˜”√∏√∫Ø ˝°£

* –Ë“™¥Úø™POWER_SAVINGµƒ∫Í∂®“Â°£
 *
 * @param   none.
 *
 * @return  none.
...
void osal_pwrmgr_powerconserve( void )
{
  ...
  // Should we even look into power conservation   ◊œ»ºÏ≤È «∑Ò÷ß≥÷µÕπ¶∫ƒ
  if ( pwrmgr_attribute.pwrmgr_device != PWRMGR_ALWAYS_ON )
  {
              
    // Are all tasks in agreement to conserve  «∑ÒÀ˘”–»ŒŒÒ÷ß≥÷µÕπ¶∫ƒ
    if ( pwrmgr_attribute.pwrmgr_task_state == 0 )
    {                                                              
//        HalLedSet(HAL_LED_1, HAL_LED_MODE_ON );   //ø™LED1    
//        HalLcdWriteString("sleep time ok", HAL_LCD_LINE_6);
      // Hold off interrupts.  πÿ÷–∂œ
      HAL_ENTER_CRITICAL_SECTION( intState );

      // Get next time-out  ≤È—Ø»Ìº˛∂® ±∆˜¡¥±Ìµ√µΩ◊ÓΩ¸“ª¥Œ“Á≥ˆ ±º‰
      next = osal_next_timeout();

      // Re-enable interrupts.  ø™÷–∂œ
      HAL_EXIT_CRITICAL_SECTION( intState );
     HalLcdWriteStringValue("next:",next, 10, HAL_LCD_LINE_5); 
      // Put the processor into sleep mode  Ω´œµÕ≥Ω¯»ÎÀØ√ﬂƒ£ Ω
      OSAL_SET_CPU_INTO_SLEEP( next );//‘⁄ OnBoard.h÷–∂®“Â»Áœ¬  #define OSAL_SET_CPU_INTO_SLEEP(m)  halSleep(m)   /* interface to HAL sleep */
    }
  }
}
#endif /* POWER_SAVING */
