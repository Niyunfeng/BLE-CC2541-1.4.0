...
#else
  HAL_KEY_SW_6_SEL &= ~(HAL_KEY_SW_6_BIT);    /* Set pin function to GPIO ÆÕÍ¨¿Ú*/
  HAL_KEY_SW_6_DIR &= ~(HAL_KEY_SW_6_BIT);    /* Set pin direction to InputÊäÈë */
...
void HalKeyPoll (void)
{
 ...
 if (!Hal_KeyIntEnable)
    {
      if (keys == halKeySavedKeys)                                                //Èç¹ûÖÐ¶Ï¹Ø±Õ£¨ÂÖÑ¯ÖÐ£©£¬²¢ÇÒ°´¼üÊÇ±£³Ö°´ÏÂµÄ
        {
        ...
        {
          //µ±°´¼üµ¯ÆðÊ±£¬¿´Ò»ÏÂ°´ÏÂµÄÊ±¼ä¶à³¤£¬ÒòÎªpollingµÄÊ±¼äÊÇ100ms£¬ËùÒÔµ±keypresslasttime<5Ê±£¬·¢³ö¶Ì°´µÄÏûÏ¢£¬
