...
// Round it to 510 seconds or 510000 ms×î´óË¯ÃßÊ±¼äÊÇ510000ms¡£
...
void halSleep( uint32 osal_timeout )
{
  ...
  // HAL_SLEEP_PM3 is entered only if the timeout is zero//Èç¹ûtimeout =0 ½øÈëÉî¶ÈË¯Ãß£¬·ñÔò½øÈëÇ³Ë¯Ãß
 ...
 // HalLcdWriteStringValue("halPwrMgtMode:",halPwrMgtMode, 10, HAL_LCD_LINE_4); 
  ...
  if ( (timeout > PM_MIN_SLEEP_TIME) || (timeout == 0) )
  {
    ...
    if ( halSleepPconValue && ( LL_PowerOffReq(halPwrMgtMode) == LL_SLEEP_REQUEST_ALLOWED ) )
    {
      ...
      // get peripherals ready for sleep //¹Ø±ÕÍâÉè£¬½øÈëË¯Ãß£¬´Ë´¦º¯ÊýÎª¿Õ
      ...
      HAL_TURN_OFF_LED3();
#else
      // use this to turn LEDs off during sleep//¹Ø±ÕLEDµÆ£¬½øÈëË¯Ãß
      HalLedEnterSleep();
#endif // HAL_SLEEP_DEBUG_LED

      // enable sleep timer interrupt//Æô¶¯Ë¯Ãß¶¨Ê±Æ÷ÖÐ¶Ï
      ...
      if (timeout != 0)
      {
        ...
        {
          // it is, so limit to max allowed sleep time (~510s)//ÉèÖÃË¯Ãß¶¨Ê±Æ÷Ê±¼ä
      ...
      HAL_SLEEP_PREP_POWER_MODE(halPwrMgtMode);//ÉèÖÃSLEEP.MODE=halPwrMgtMode,PCON.IDLE=1
...
#if ((defined HAL_KEY) && (HAL_KEY == TRUE))
      // handle peripherals
      (void)HalKeyExitSleep();//Èç¹ûÓÐ°´¼ü°´ÏÂ£¬Ìø³öË¯ÃßÄ£Ê½£¬Ê¹ÄÜËùÓÐÖÐ¶Ï
#endif // ((defined HAL_KEY) && (HAL_KEY == TRUE))
    }
